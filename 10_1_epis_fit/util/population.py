...
def init_first_cases(state:torch.tensor, preN= -1):
    kn= state.shape[1]
    n= state.shape[2] if preN== -1 else preN
    for i in range(kn):
        firstInfected= random.randint(0, n-1)
        state[0, i, firstInfected, 2, 0]= 0.01 #go to hidden layer
        state[0, i, firstInfected, 0, 0]= 0.99